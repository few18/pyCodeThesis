...
def DeepESN_real(X, Y, n_data, n_surrogates = 200, verbose = False):
...
# import data
SardineAnchovy = pd.read_csv('sardine_anchovy_sst.csv')
combs = np.loadtxt('combs_sardineanchovy.csv', delimiter = ',', dtype = str)

#get index of combination to run
parser = argparse.ArgumentParser(description="Sardine Anchovy")
parser.add_argument(
...
X = np.array(SardineAnchovy[X_name])
Y = np.array(SardineAnchovy[Y_name])
...
xmapy_surr, ymapx_surr, xmapy, ymapx = DeepESN_real(X, Y, n_data = 200, n_surrogates = 200)
